...
// FIX 1: kernel parameter was named h_c (host pointer) but receives a device
// output pointer d_c — renamed to d_c to match the call sites and avoid
// confusion.  The kernel itself writes to GPU memory; it is not mapped memory.
// FIX 2: kernel was named "pow" which shadows std::pow used inside the body,
// causing infinite recursion / compile error.  Renamed to power_kernel.
__global__ void power_kernel(float *d_a, float *d_b, float *d_c, int numElements)
{
...
__host__ std::tuple<float *, float *> allocateRandomHostMemory(int numElements)
{
    ...
    // FIX 3: h_a must be pageable (regular) host memory — use malloc
    float *h_a;
    h_a = (float *)malloc(size);

    // FIX 4: h_b must be pinned host memory — use cudaMallocHost
    float *h_b;
...
__host__ std::tuple<float *, float *, int> readCsv(std::string filename)
{
    ...
    // FIX 5: h_a in readCsv must also be pageable memory — use malloc
    float *h_a;
    h_a = (float *)malloc(size);
    ...
    // FIX 6: h_b in readCsv must be pinned memory — use cudaMallocHost
    float *h_b;
    cudaMallocHost((float **)&h_b, size);
...
__host__ std::tuple<float *, float *> allocateDeviceMemory(int numElements)
{
    ...
    // FIX 7: incomplete assignment "cudaError_t err =" — add cudaMalloc call
    ...
    float *d_b;
    // FIX 8: incomplete assignment "err =" — add cudaMalloc call
...
{
    // FIX 9: size must use sizeof(float), not sizeof(int) — these are float arrays
    size_t size = numElements * sizeof(float);

    // FIX 10: "cudaGMemcpy" is not a real CUDA function — changed to cudaMemcpy
...
{
    // FIX 11: d_c must be allocated on the device to receive kernel output,
    // then the result is copied back to h_c (unified memory) after the kernel.
    float *d_c;
    cudaError_t err = cudaMalloc(&d_c, numElements * sizeof(float));
    if (err != cudaSuccess)
    {
    ...
    // FIX 12: was calling "div" (which doesn't exist in this file) — call power_kernel
    power_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_a, d_b, d_c, numElements);
    ...
    // Copy result from device d_c back to host h_c (unified memory)
