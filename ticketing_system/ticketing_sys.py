...
lock = Lock()
current_ticket = [0]        # mutable shared variable
...
class Assignment(Core):
    ...
    def run(self):
        ...
        if self.test_username_equality(self.USERNAME):
            ...
            for index in range(self.num_threads):
                ...
                # Start thread participating in ticketing system
                t = Thread(
                    target=execute_ticketing_system_participation,
                    args=(index, self.part_id, current_ticket)
                )
    ...
    def manage_ticketing_system(self):
        ...
        # increment shared ticket while active threads exist
        while True:
            with lock:
